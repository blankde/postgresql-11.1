...
eval_windowaggregates_sample(WindowAggState *winstate)
{
	...
	if(winstate->numindex != 0){
		/*
		while(winstate->samindex[winstate->indexheadpos%samplesize] < winstate->frameheadpos)
		{
			++(winstate->indexheadpos);
			--(winstate->numindex);
		}
		...
		{
			int			ret;
			if(peraggstate->restart && !isNew && cnt < winstate->numindex)
			{
